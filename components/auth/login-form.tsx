...
import { useAuth, testCredentials } from "@/contexts/auth-context"
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md mx-auto">
      ...
      >
        {loading ? (
          ...
          <span className="flex items-center justify-center gap-2">{GOOGLE_ICON} Sign in with Google</span>
        )}
      </Button>
      ...
      {process.env.NODE_ENV === "development" && (
        <div className="mt-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <p className="text-xs text-blue-700">User: {testCredentials.user.email}</p>
          <p className="text-xs text-blue-700">Password: {testCredentials.user.password}</p>
        </div>
      )}
    </motion.div>
  )
